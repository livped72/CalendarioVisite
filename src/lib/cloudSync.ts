...
export function setSavedSyncCode(code: string): void {
  if (code.trim()) {
...
export async function pushDataToCloud(
  ...
  const plainPayload: CalendarioData = {
...
export async function pullDataFromCloud(
  ...
  try {
    ...
    let decryptedData: CalendarioData;

    // Se è un pacchetto cifrato E2EE
    if (cloudResponse && cloudResponse.isEncrypted && cloudResponse.payload) {
      try {
        decryptedData = await decryptData<CalendarioData>(cloudResponse.payload, pwd);
      } catch (decErr: any) {
        return {
          success: false,
