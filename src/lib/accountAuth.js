...
async function computeAuthVerifier(password, email) {
...
function saveLocalAccount(key, record) {
    try {
...
export async function registerAccount(email, password, nome) {
    ...
    try {
        ...
        const profile = { email: normalizedEmail, nome: nome.trim() };
...
export async function loginAccount(email, password) {
    ...
    try {
        const decryptedData = await decryptData(record.encryptedData, password);
 ...
 */
export async function pullAccountData() {
    const user = getCurrentUser();
    const password = getSessionPassword();
    if (!user || !password)
        return { success: false, error: 'Utente non autenticato.' };
    ...
    try {
        ...
        if (!res.ok)
            return { success: false, error: 'Nessun dato aggiornato sul server.' };
        const record = await res.json();
        const decrypted = await decryptData(record.encryptedData, password);
 ...
 */
export function getAccountLastSyncTime() {
