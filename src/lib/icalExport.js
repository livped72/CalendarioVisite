 ...
 */
function getNextDayICal(dateStr) {
    try {
 ...
 */
export function generateICalendar(settimane, appuntamenti = [], congregazioni = []) {
    const nowStamp = new Date().toISOString().replace(/[-:]/g, '').split('.')[0] + 'Z';
    const congMap = new Map(congregazioni.map((c) => [c.id, c]));
    const lines = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
    ...
    for (const a of appuntamenti) {
        ...
        lines.push(`DESCRIPTION:${description}`);
        if (a.luogo)
            lines.push(`LOCATION:${escapeICalText(a.luogo)}`);
 ...
 */
export function downloadICalFile(filename, icsContent) {
