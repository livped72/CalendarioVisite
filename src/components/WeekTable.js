...
export const WeekTable = ({ settimane, onEditWeek, onDeleteWeek, onDuplicateWeek, visitNumberMap, onViewAppuntamenti, }) => {
    ...
    const getVisitInfo = (item) => {
        ...
        for (const w of settimane) {
            if (w.evento === 'congregazione') {
                localNum++;
                if (w.id === item.id)
                    return { numero: localNum };
            }
        }
        return { numero: 1 };
    };
    const getBadgeColor = (visitNum) => BADGE_COLORS[(Math.max(1, visitNum) - 1) % BADGE_COLORS.length];
    ...
    return (_jsxs("div", { className: "bg-white rounded-2xl border border-[#E0DED9] shadow-2xs overflow-hidden", children: [_jsx("div", { className: "hidden lg:block overflow-x-auto", children: _jsxs("table", { className: "w-full text-left text-xs border-collapse", children: [_jsx("thead", { children: _jsxs("tr", { className: "border-b border-[#E0DED9] bg-[#FAF9F7] text-[#2F3332] font-bold uppercase tracking-wider text-[11px]", children: [_jsx("th", { className: "py-3 px-4 w-16 text-center", children: "#" }), _jsx("th", { className: "py-3 px-4 w-52", children: "Periodo (Mar \u2013 Dom)" }), _jsx("th", { className: "py-3 px-4 w-64", children: "Evento" }), _jsx("th", { className: "py-3 px-4", children: "Note" }), _jsx("th", { className: "py-3 px-3 w-16 text-right pr-4" })] }) }), _jsx("tbody", { className: "divide-y divide-[#EFECE6] text-[#2F3332]", children: displayItems.map((item) => {
                                ...
                                return (_jsxs("tr", { className: "hover:bg-[#FAF9F7] transition-colors group", children: [_jsx("td", { className: "py-3 px-4 text-center", children: isCong && visitNum != null ? (_jsx("span", { className: `inline-flex items-center justify-center w-6 h-6 rounded-full text-white text-[11px] font-bold shadow-2xs ${getBadgeColor(visitNum)}`, title: visitInfo?.motivazione || `Visita #${visitNum}`, children: visitNum })) : (_jsx("span", { className: "inline-flex items-center justify-center w-6 h-6 rounded-full bg-[#F3F0EA] text-[#AAA] text-[10px]", children: "\u2014" })) }), _jsx("td", { className: "py-3 px-4 font-bold text-[#2F3332] whitespace-nowrap", children: onViewAppuntamenti ? (_jsxs("button", { type: "button", onClick: () => onViewAppuntamenti(item.id), className: "group/cell inline-flex items-center gap-1.5 font-bold text-[#2F3332] hover:text-[#5B6760] transition-colors cursor-pointer", title: "Vedi appuntamenti di questa settimana", children: [abbreviateMonths(item.periodo), _jsx(CalendarDays, { className: "w-3.5 h-3.5 text-[#7C8B82] opacity-0 group-hover/cell:opacity-100 transition-opacity shrink-0" })] })) : (abbreviateMonths(item.periodo)) }), _jsx("td", { className: "py-3 px-4 whitespace-nowrap", children: onViewAppuntamenti ? (_jsx("button", { type: "button", onClick: () => onViewAppuntamenti(item.id), className: "cursor-pointer", title: "Vedi appuntamenti di questa settimana", children: renderEventCell(item) })) : (renderEventCell(item)) }), _jsx("td", { className: "py-3 px-4 text-[#666] text-xs", children: item.note && item.note !== '-'
                                                ? item.note
                    ...
                    const visitNum = visitInfo?.numero;
                    const isCong = item.evento === 'congregazione';
                    return (_jsxs("div", { className: "p-3.5 hover:bg-[#FAF9F7] active:bg-[#F7F5F0] transition-colors flex items-center justify-between gap-3", children: [_jsxs("div", { className: "flex items-center gap-3 min-w-0 flex-1 cursor-pointer", onClick: () => onViewAppuntamenti ? onViewAppuntamenti(item.id) : onEditWeek(item), title: onViewAppuntamenti ? 'Vedi appuntamenti di questa settimana' : 'Modifica', children: [isCong && visitNum != null ? (_jsx("div", { className: "flex flex-col items-center shrink-0", children: _jsx("span", { className: `w-7 h-7 rounded-xl text-white text-xs font-extrabold flex items-center justify-center shadow-2xs ring-2 ring-white ${getBadgeColor(visitNum)}`, children: visitNum }) })) : (_jsx("span", { className: "w-7 h-7 rounded-xl bg-[#F0EEE8] text-[#888] text-xs font-bold flex items-center justify-center shrink-0 ring-2 ring-white", children: "\u2014" })), _jsxs("div", { className: "min-w-0 flex-1", children: [_jsxs("div", { className: "flex items-center gap-2 mb-1", children: [_jsx("span", { className: "text-[11px] font-bold text-[#7C8B82] uppercase tracking-wider", children: abbreviateMonths(item.periodo) }), visitInfo?.isReset && (_jsx("span", { className: "text-[9px] font-extrabold text-[#7C8B82] uppercase bg-[#EBF1ED] px-1.5 py-0.5 rounded-md", children: "Ciclo 1" }))] }), _jsx("div", { className: "flex items-center gap-1.5", children: renderEventCell(item) }), item.note && item.note !== '-' && (_jsx("div", { className: "text-[11px] text-[#777] truncate mt-1", children: item.note }))] })] }), _jsxs("div", { className: "flex items-center gap-1 shrink-0 pl-1", children: [onViewAppuntamenti && (_jsx("button", { onClick: (e) => {
                                            e.stopPropagation();
                                            onEditWeek(item);
                                        }, className: "p-2 rounded-xl text-[#777] hover:text-[#2F3332] hover:bg-[#FAF9F7] active:bg-[#EFECE6] transition-colors cursor-pointer", title: "Modifica settimana", "aria-label": "Modifica settimana", children: _jsx(Pencil, { className: "w-4 h-4" }) })), _jsx("div", { onClick: () => onViewAppuntamenti ? onViewAppuntamenti(item.id) : onEditWeek(item), className: "p-1 cursor-pointer text-[#BBB] hover:text-[#5B6760] transition-colors", children: _jsx(ChevronRight, { className: "w-4 h-4" }) })] })] }, item.id));
